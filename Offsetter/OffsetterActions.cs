...
namespace Offsetter
{
    ...
    public partial class Offsetter : Form
    {
        ...
        {
            OffsetDialog dialog = new OffsetDialog("Uniform Offset", offsetSide, offsetDist);
            if (dialog.ShowDialog() != DialogResult.OK)
                return;
            ...
            GOffsetter offsetter = new GOffsetter();
        ...
        private void ConvexHullOffset()
        {
            OffsetDialog dialog = new OffsetDialog("Convex Hull Offset", offsetSide);
            if (dialog.ShowDialog() != DialogResult.OK)
                return;
            ...
            GChainList results = new GChainList();
            GConvexHull ch = new GConvexHull(false);
        ...
        private void Nest()
        {
            OffsetDialog dialog = new OffsetDialog("Nest", offsetSide);
            if (dialog.ShowDialog() != DialogResult.OK)
                return;
            ...
            GChainList results = new GChainList();
            GConvexHull ch = new GConvexHull(true);
            ch.Offset(Part, Tool, offsetSide, 0, results);
            if (results.Count <= 0)
                return;
